%% Training 1st one
%clear all
intIm = imread('intersect.png');
bumpIm = imread('bump.png');
yieldIm = imread('giveway.png');
mergeIm = imread('lane.png');

%cd ./Training
cd ./Stop
folder = num2str(17,'%05i');
cd(folder)
...
%% %% TRAINIGN FOR FOURTH SIGN
cd ..
folder = num2str(14,'%05i');
cd(folder)
files.filename = ls('*ppm');
names = files.filename;
Size4 = size(names);
trainingFeatures4 = zeros(Size4(1), hogFeatureSize,'single');
%trainingFeatures = [trainingFeatures ; trainingFeaturesAdd];
for i = 1:Size4(1)
    curr_name = names(i,:);
    img = imread(curr_name);
  ...
  %  imshow(crop)
    ...
    trainingFeatures4(i,:) = extractHOGFeatures(B, 'CellSize', cellSize);
   % [i Size(1)]
   if (i == 26)
   ident4 = crop;
   end
end
Size4 = size(trainingFeatures4);
trainingLabels4 = ones(Size4(1),1);
index = (trainingLabels4 == 1);
trainingLabels4(index) = 3;
...
trainingFeatures = vertcat(trainingFeatures1,trainingFeatures2, trainingFeatures3,trainingFeatures4);
trainingLabels = vertcat(trainingLabels1, trainingLabels2,trainingLabels3,trainingLabels4);
